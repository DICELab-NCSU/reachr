...
dnld_flow360 <- function(dir = "~/Downloads", overwrite = FALSE,
                         usr = Sys.getenv("EMLID_USER"), pwd = Sys.getenv("EMLID_PASSWORD")) {
  # set browser profile
  ...
  # start Selenium
  rD <- rsDriver(browser = "firefox", port = 4567L, verbose = FALSE, phantomver = NULL,
  ...
  for (i in seq_len(max_scrolls)) {
    ...
    for (tile in tiles) {
      ...
      if (!is.null(final_export_btn)) {
        ...
        Sys.sleep(4)  # wait for download to start
